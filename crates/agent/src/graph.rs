...
pub fn get_tools() -> Vec<ToolInfo> {
    vec![
        ...
        ToolInfo::new(
            "disktracker_search",
            "Searches indexed files/directories. Default is exact case-insensitive substring match. Set advanced=true (CLI: --advanced/--advance) for multi-tier scoring (phrase, prefix, ngram, fuzzy). Set fuzzy=true (CLI: --fuzzy) to enable the fuzzy matching tier (also implies advanced).",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "query": {
                    ...
                    "advanced": {
                        "type": "boolean",
                        "description": "Enable multi-tier scoring (phrase, prefix, ngram, fuzzy). Same as CLI --advanced / --advance."
                    },
                    "fuzzy": {
                        "type": "boolean",
                        "description": "Enable fuzzy matching scoring tier (typo tolerance). Same as CLI --fuzzy. Also enables advanced multi-tier search."
                    },
                    "limit": {
                        "type": "integer",
        ...
        ToolInfo::new(
            "disktracker_history",
            "Queries the mutation history of a specific file or directory. Created/Deleted/Renamed always shown (including 0B); Modified only when |size_delta| >= 2. Consecutive identical Created/Deleted/Renamed rows for the same file are aggregated; Modified rows stay unique.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "path": {
                    ...
                    "collapse": {
                        "type": "boolean",
                        "description": "Also merge consecutive same-file same-kind rows (including Modified size deltas). Identical Created/Deleted/Renamed rows are already aggregated without this flag."
                    },
                    "limit": {
                        "type": "integer",
        ...
        ToolInfo::new(
            "disktracker_top",
            "Ranks files/folders by size, growth, or churn. Growth mode: 0B create/delete events can appear; tiny non-zero growth under 2B is hidden (same floor as Modified elsewhere).",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "volume": {
                    ...
                    "growth": {
                        "type": "boolean",
                        "description": "Rank by size delta (growth). Shows 0B create/delete; hides |delta| of 1B."
                    },
                    "churn": {
                        "type": "boolean",
        ...
        ToolInfo::new(
            ...
            "Diffs two snapshots to see net file mutations. Created/Deleted/Renamed always shown (including 0B); Modified only when |size_delta| >= 2.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "snapshot_a": {
    ...
    graph.add_node("planner", |mut state: AskState, _config: &Config| async move {
        ...
        let system_prompt = format!(
            "You are AI AGENT, a natural-language orchestration agent for DiskTracker.\n\
             ...
             - mutation_log display rules (history, snapshot_diff, top growth): Created/Deleted/Renamed are always shown including 0B; Modified requires |size_delta| >= 2. History also aggregates consecutive identical Created/Deleted/Renamed rows for the same file; Modified rows stay unique.\n\
             \n\
             DYNAMIC CONTEXT:\n\
             - Shell Mode: {}\n\
             - Operating System: {}\n\
            ...
            for tc in tool_calls {
                ...
                match tool_name {
                    ...
                    "disktracker_search" => {
                        ...
                        let advanced = tc.arguments.get("advanced").and_then(|a| a.as_bool()).unwrap_or(false);
                        let fuzzy = tc.arguments.get("fuzzy").and_then(|f| f.as_bool()).unwrap_or(false);
