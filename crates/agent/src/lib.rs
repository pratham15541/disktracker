...
pub fn set_api_key(key: &str) -> Result<(), String> {
    if cfg!(windows) {
        ...
        entry.set_password(key).map_err(|e| {
            format!(
                "Failed to store API key in Windows Credential Manager: {:?}",
                e
            )
        })?;
    } else {
...
pub fn get_api_key() -> Result<String, String> {
    if cfg!(windows) {
        ...
        entry.get_password().map_err(|e| {
            format!(
                "Failed to retrieve API key from Windows Credential Manager: {:?}",
                e
            )
        })
    } else {
        ...
        if !key_path.exists() {
            return Err(
                "API key not found. Run 'disktracker ai config --api-key <key>' to set it."
                    .to_string(),
            );
        }
...
pub fn check_ai_configuration_validity() -> Result<(String, String, String), String> {
    ...
    let model = match config.ai_model {
        ...
        _ => return Err("no model named added".to_string()),
    };
    let api_key = match get_api_key() {
        Ok(k) if !k.trim().is_empty() => k.trim().to_string(),
        _ => return Err("no api key present".to_string()),
    };
...
pub fn set_websearch_api_key(key: &str) -> Result<(), String> {
    if cfg!(windows) {
        let entry = Entry::new("disktracker-ai", "websearch-api-key")
            .map_err(|e| format!("Failed to create keyring entry: {:?}", e))?;
        entry.set_password(key).map_err(|e| {
            format!(
                "Failed to store Websearch API key in Windows Credential Manager: {:?}",
                e
            )
        })?;
    } else {
        // Fallback for WSL/Linux
        let mut key_path = storage::get_db_dir().map_err(|e| e.to_string())?;
        key_path.push("ai_websearch_key");
        fs::write(&key_path, key)
            .map_err(|e| format!("Failed to write Websearch AI key fallback: {:?}", e))?;
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let _ = fs::set_permissions(&key_path, fs::Permissions::from_mode(0o600));
...
pub fn get_websearch_api_key() -> Result<String, String> {
    if cfg!(windows) {
        let entry = Entry::new("disktracker-ai", "websearch-api-key")
            .map_err(|e| format!("Failed to create keyring entry: {:?}", e))?;
        entry.get_password().map_err(|e| {
            format!(
                "Failed to retrieve Websearch API key from Windows Credential Manager: {:?}",
                e
            )
        })
    } else {
        ...
        let key = fs::read_to_string(&key_path)
            .map_err(|e| format!("Failed to read Websearch AI key fallback: {:?}", e))?;
        Ok(key.trim().to_string())
    }
}
...
pub fn update_ai_config(
    ...
    if let Some(provider) = websearch_provider {
        config.ai_websearch_provider = Some(provider);
    }
    if let Some(cx) = websearch_cx {
        config.ai_websearch_cx = Some(cx);
    }
    save_config(&config)
}
...
pub async fn query_daemon_rpc(
    ...
    let response: serde_json::Value =
        serde_json::from_str(&line).map_err(|e| format!("Failed to parse JSON response: {}", e))?;
        ...
        match query_daemon_rpc(method, serde_json::json!({ "command": command })).await {
            ...
            Err(e) => {
                if e.starts_with("E_COMMAND_NOT_WHITELISTED: ") {
                    let first_word = e
                        .trim_start_matches("E_COMMAND_NOT_WHITELISTED: ")
                        .trim()
                        .to_string();

                    println!(
                        "\n[Security] Command '{}' is not in the {} whitelist.",
                        first_word, mode
                    );
                    print!(
                        "Should I add '{}' to the {} whitelist and execute it? [y/N]: ",
                        first_word, mode
                    );
...
pub async fn run_agent_query(
    ...
    let mut graph =
        graph::build_agent_graph().map_err(|e| format!("Failed to build agent graph: {}", e))?;

    let state = match initial_state {
        Some(mut s) => {
            s.question = question.to_string();
            s.messages
                .push(rust_langgraph::state::Message::user(question));
    ...
    let result = graph
        .invoke(state, rust_langgraph::config::Config::default())
        .await
