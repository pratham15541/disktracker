...
pub async fn run_server(
    ...
    let db_path = storage::init_db().map_err(|e| io::Error::other(e.to_string()))?;
    println!("[Daemon] SQLite Database initialized at {:?}", db_path);

    // Start ETW (real or mock) process & telemetry tracking engine
    platform_windows::start_etw_engine();

    let conn = storage::get_db_connection().map_err(|e| io::Error::other(e.to_string()))?;
...
#[cfg(windows)]
fn get_pe_metadata(path: &str) -> Option<(String, String)> {
    ...
    let mut dw_handle = 0;
    let size = unsafe { GetFileVersionInfoSizeW(path_w.as_ptr(), &mut dw_handle) };
    if size == 0 {
        return None;
    }
    ...
    let mut lp_buffer = std::ptr::null_mut();
    let mut len = 0;
    let trans_subblock: Vec<u16> = std::ffi::OsStr::new(r"\VarFileInfo\Translation")
        .encode_wide()
        .chain(std::iter::once(0))
        .collect();
    ...
    unsafe {
            ...
            && len >= 4
        {
            ...
            lang_code = format!("{:04x}{:04x}", lang, codepage);
        }
    }
...
#[cfg(not(windows))]
fn get_pe_metadata(_path: &str) -> Option<(String, String)> {
    None
}

fn resolve_signature(target: &str) -> String {
    let target_lower = target.to_lowercase();

    // 1. Check Database footprint associations (installer or runtime telemetry)
    if let Ok(Some((name, mode))) = storage::find_footprint_association(target) {
        if mode == "install" {
            return format!("This path is an installation footprint of application '{}' (resolved dynamically via installer telemetry).", name);
        } else {
            return format!("This path is a runtime artifact generated by process '{}' (resolved dynamically via runtime telemetry).", name);
        }
    }

    // 2. Check signatures.json overrides
    if let Ok(serde_json::Value::Object(map)) = storage::load_signatures() {
        for (key, val) in map.iter() {
            if target_lower.contains(&key.to_lowercase()) {
    ...
    // 3. Check PE executable version resource details
    if target_lower.ends_with(".exe") {
        if let Some((company, product)) = get_pe_metadata(target) {
            return format!("Executable published by '{}', part of product '{}' (resolved dynamically from file headers).", company, product);
        }
    }

    format!("Unknown signature/path '{}'. No installation or runtime footprints associated with this path.", target)
}

fn is_command_safe_read_only(cmd: &str) -> Result<(), String> {
    let forbidden = [";", "&", "|", ">", "<", "$(", "`"];
    ...
    let whitelist = match storage::load_whitelist() {
        Ok(serde_json::Value::Object(map)) => {
            if let Some(serde_json::Value::Array(arr)) = map.get("read") {
                arr.iter()
    ...
    if !whitelist.contains(&first_word) {
        return Err(format!("E_COMMAND_NOT_WHITELISTED: {}", first_word));
    }

    Ok(())
}

fn execute_read_only_command(cmd: &str) -> Result<String, String> {
    is_command_safe_read_only(cmd)?;
...
fn is_command_safe_write(cmd: &str) -> Result<(), String> {
    let forbidden = [";", "&", "|", ">", "<", "$(", "`"];
    for char_str in forbidden {
        if cmd.contains(char_str) {
            return Err(format!(
    ...
    let whitelist = match storage::load_whitelist() {
        Ok(serde_json::Value::Object(map)) => {
            if let Some(serde_json::Value::Array(arr)) = map.get("write") {
                arr.iter()
                    .filter_map(|v| v.as_str())
                    .map(|s| s.to_lowercase())
                    .collect::<Vec<String>>()
...
fn execute_write_command(cmd: &str) -> Result<String, String> {
    is_command_safe_write(cmd)?;
...
async fn handle_request(
    ...
    match req.method.as_str() {
        "status" => {
            ...
            let aggregate_state = if volumes.is_empty() || all_live {
                DaemonState::Live
            } else {
